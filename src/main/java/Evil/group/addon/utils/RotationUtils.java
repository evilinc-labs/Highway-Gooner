...
// imports to test update to 1.21.5 mappings
...
public class RotationUtils {
    ...
    // start of modified onTickPost with InputAccessor
    @EventHandler
    public void onTickPost(TickEvent.Post event) {
        if (rotation != null && mc.player != null && movementFix) {
            // Read current input vector (x = sideways/strafe, y = forward)
            ...
            // Keep original rounding behavior
            newSideways = Math.round(newSideways);
            newForward  = Math.round(newForward);

            // Write back to Input.movementVector
            ((InputAccessor) mc.player.input).setMovementVector(new Vec2f(newSideways, newForward));
        }
    }    
    // end modified onTickPost
    ...
    private void lookAt(Vec3d target) {
        ...
        float[] rot = RotationUtils.getRotationsTo(eye, target);
